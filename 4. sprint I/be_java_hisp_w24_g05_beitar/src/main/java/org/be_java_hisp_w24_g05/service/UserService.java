...
public class UserService implements IUserService {
    ...
    @Override
    public ProductPromoCountDto getUserPromoProducts(Integer userId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        int productCount = (int) user.getPosts().stream().filter(Post::getHasPromo).map(Post::getProduct).count();
        if (productCount == 0) throw new BadRequestException("User ID: " + userId + " doesn't have products with promo.");
        return convertProductPromoCountDto(user, productCount);
    }
    ...
    private ProductPromoCountDto convertProductPromoCountDto(User u, Integer counted){
        return new ProductPromoCountDto(
                u.getUserId(),
                u.getUserName(),
                counted
        );
