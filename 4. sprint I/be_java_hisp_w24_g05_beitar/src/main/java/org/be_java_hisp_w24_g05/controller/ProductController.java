...
import org.be_java_hisp_w24_g05.dto.ProductPromoCountDto;
...
public class ProductController {
    ...
    public ResponseEntity<?> savePostPromo(@RequestBody PostPromoDto p){
        return new ResponseEntity<>(productService.savePostPromo(p), HttpStatus.OK);
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<ProductPromoCountDto> getCountPromoProducts(@RequestParam(name = "user_id") int userId){
        return new ResponseEntity<>(userService.getUserPromoProducts(userId), HttpStatus.OK);
    }
}
