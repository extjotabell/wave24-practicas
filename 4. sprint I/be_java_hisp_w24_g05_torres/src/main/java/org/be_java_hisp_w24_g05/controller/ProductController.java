...
public class ProductController {
    ...
    public ResponseEntity<?> countPromoPosts(@RequestParam(name = "userId", defaultValue = "") int userId){
        return ResponseEntity.ok(productService.countPromoPosts(userId));
    }

    @GetMapping("/post/{postId}/favorite/{userId}")
    public ResponseEntity<?> favoritePost(@PathVariable int postId, @PathVariable int userId){
        return ResponseEntity.ok(productService.favoritePost(postId, userId));
    }

    @GetMapping("/post/favorite/{userId}/list")
    public ResponseEntity<?> listFavoritePosts(@PathVariable int userId){
        return ResponseEntity.ok(productService.listFavoritePosts(userId));
    }
