...
public class UserRepository implements IUserRepository{
    ...
    public Optional<User> findById(Integer id) {
        User user = this.users.stream().filter(u -> u.getUserId().equals(id)).findFirst().orElseThrow(() -> new BadRequestException("User not found"));
    ...
    public User addPost(Post post) {
        User user = this.users.stream().filter(u -> Objects.equals(u.getUserId(), post.getUserId())).findFirst().orElse(null);
        if(Objects.isNull(user) ) throw new BadRequestException("User does not exist");
        Integer postId = user.getPosts().size()+1;
    ...
    public List<Post> getPromoPostbyUserId(int userId) {
        ...
        return user.getPosts().stream().filter(post -> post instanceof PromoPost).toList();
    }
