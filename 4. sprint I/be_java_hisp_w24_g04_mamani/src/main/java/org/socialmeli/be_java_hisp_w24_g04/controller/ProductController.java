...
public class ProductController {
    ...
    public ResponseEntity<SingleResponseDTO> createPromoPost(@RequestBody PromoPostDTO promoPostDTO) {
        return ResponseEntity.ok(
                new SingleResponseDTO(200, postService.createPromoPost(promoPostDTO))
        );
    }

    @GetMapping("/promo-post/count")
    public ResponseEntity<SingleResponseDTO> getPromoPostCount(@RequestParam("user_id") Integer userId) {
        return ResponseEntity.ok(
                new SingleResponseDTO(200, postService.getPromoPostCount(userId))
        );
    }

    @GetMapping("/promo-post/search")
    public ResponseEntity<SingleResponseDTO> searchPromoPosts(@RequestParam(value = "brand", required = false) String brand, @RequestParam(value = "type", required = false) String type) {
        return ResponseEntity.ok(
                new SingleResponseDTO(200, postService.searchPromoPosts(brand, type))
        );
    }
}
