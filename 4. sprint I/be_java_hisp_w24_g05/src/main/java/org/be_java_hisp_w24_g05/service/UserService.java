...
public class UserService implements IUserService {
    ...
    @Override
    public CountPromoPostDto countPromoPost(Integer userId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        return userToCountPromoPostDTO(user);
    }
    ...
    private CountPromoPostDto userToCountPromoPostDTO(User user) {
        return new CountPromoPostDto(
                user.getUserId(),
                user.getUserName(),
                user.getPosts().stream().filter(Post::getHasPromo).toList().size()
        );
    }
