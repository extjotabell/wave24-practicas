...
public class SocialController {
    ...
    public ResponseEntity<UserProductsDto> getTotalUserProducts(@RequestParam("user_id") Integer userId){
        return new ResponseEntity<>(postService.getTotalProductsProm(userId), HttpStatus.OK);
    }

    @PostMapping("/products/promo-post")
    public ResponseEntity<?> createPostProm(@RequestBody PostPromDto postPromDto){
        return new ResponseEntity<>(postService.savePostProm(postPromDto), HttpStatus.OK);
    }

    @GetMapping("/products/categories/{userId}")
    public ResponseEntity<UserCategoriesDto> getProductsForCategory(@PathVariable("userId") Integer userId){
        return new ResponseEntity<>(postService.getProductsForCategory(userId), HttpStatus.OK);
    }
