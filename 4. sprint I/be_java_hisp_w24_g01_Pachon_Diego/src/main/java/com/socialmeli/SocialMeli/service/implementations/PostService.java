...
import com.socialmeli.SocialMeli.entity.*;
...
public class PostService implements IPostService {
    ...
    public void createPromoPost(PostPromoRequestDTO postDTO) {
        // Validar el DTO
        Objects.requireNonNull(postDTO, "Error creating the promo post");
        Category category = categoryRepository.findById(postDTO.category())
                .orElseThrow(() ->
                        new NotFoundException("The category id: " + postDTO.category() + " not found"));
        // Verificar si ya existe un post con el mismo usuario e ID de producto
        postRepository.findById(postDTO.user_id() + 1)
                .filter(existingPost -> existingPost.getProduct().getId().
                        equals(postDTO.product().product_id()))
    ...
    @Override
    public PromoCountDTO countPostsPromo(Integer  id) {
        List<Post> posts = postRepository.countPostsPromo(id);
        if (posts.isEmpty())
            throw new EmptyListException("Promo posts not found");
        User user = userRepository.findById(id)
                .orElseThrow(() -> new UserNotFoundException("The user id: " + id + " not found"));
        return new PromoCountDTO(id, user.getName(), posts.size());
    }
