...
public class UserService implements IUserService {
    ...
    @Override
    public CountPromoPostsDto searchUserPromoPosts(Integer userId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        return userToCountPromoPostsDTO(user);
    }
    ...
    private CountPromoPostsDto userToCountPromoPostsDTO(User user) {
        if (user.getPosts()==null) throw new NotFoundException("User ID: " + user.getUserId() + " doesn't have any post.");

        return new CountPromoPostsDto(
                user.getUserId(),
                user.getUserName(),
                user.getPosts().stream().filter(Post::getHasPromo).count()
        );
    }
}
