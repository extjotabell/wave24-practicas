...
public class PostService implements IPostService{
    ...
    public PromoPostDto savePromoPost(PromoPostDto promoPostDto) {
        if(Objects.isNull(promoPostDto)){
            throw new BadRequestException("Los datos ingresados no son correctos.");
        }
        User user = userRepository.findById(promoPostDto.user_id()).orElse(null);
        if(Objects.isNull(user)){
            throw new BadRequestException("No existe el usuario con id: " + promoPostDto.user_id());
        }
        Post promoPost = mapper.convertPromoPostDtoToPost(promoPostDto);
        var promoPostList = postRepository.save(promoPost);
        return mapper.convertPostToPromoPostDto((Post) promoPostList);
    }

    @Override
    public PromoCountDto getTotalPromoProducts(Integer userId) {
        User user = userRepository.findById(userId).orElse(null);
        if (Objects.isNull(user)){
            throw new BadRequestException("Usuario con id " + userId + " no existe.");
        }
        var promoProductsCount = postRepository.findAll().stream()
                .filter(promoPost -> promoPost.getUserId().equals(userId))
                .filter(promoPost -> promoPost.getHasPromo().equals(true)).collect(Collectors.toList()).size();
        return new PromoCountDto(userId, user.getUserName(), (int) promoProductsCount);
    }
