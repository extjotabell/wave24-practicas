...
public class SaleService implements ISaleService {
    ...
    private Stream<Sale> iterableToStream(Iterable<Sale> iterable) {
    ...
    public SaleListResponseDTO findAllSales() {
        return new SaleListResponseDTO(
                iterableToStream(saleRepository.findAll()).map(SaleResponseDTO::of).toList()
        );
    }
    ...
    public SaleResponseDTO findOneSale(Integer id) {
        return SaleResponseDTO.of(
                saleRepository.findById(id.toString())
    ...
    public Boolean updateSale(Integer code, SaleRequestDTO saleRequestDTO) {
        var saleToUpdate = saleRepository.findById(code.toString());
        ...
        var newClothes = saleRequestDTO.clothes()
                .stream()
                .map(cloth -> {
                    var clothFound = clothRepository.findById(cloth.id().toString())
                            .orElseThrow(
                                    () -> new NotFoundException("The cloth you are looking for does not exist")
                            );
    ...
    public Boolean deleteSale(Integer id) {
        try {
            var saleToDelete = saleRepository.findById(id.toString());
            ...
            saleRepository.deleteById(id.toString());
        } catch (Exception e) {
            return false;
        }
    ...
    public ClothesListResponseDTO searchClothesBySaleId(Integer id) {
        var sale = saleRepository.findById(id.toString());
