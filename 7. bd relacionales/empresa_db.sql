...
CREATE TABLE empresa_db.empleado (
	...
	comision INT DEFAULT 0 NULL,
	depto_nro varchar(20) NOT NULL,
	CONSTRAINT NewTable_PK PRIMARY KEY (cod_emp)
)
ENGINE=InnoDB
...
CREATE TABLE empresa_db.departamento (
	...
	CONSTRAINT departamento_pk PRIMARY KEY (depto_nro)
)
ENGINE=InnoDB
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_0900_ai_ci;

-- Crear Foreing Key en tabla Empleado
ALTER TABLE empresa_db.empleado
    ADD CONSTRAINT empleado_departamento_FK
    FOREIGN KEY (depto_nro)
    REFERENCES enterprise_db.departamento(depto_nro);
...
SELECT e.nombre nombre, e.puesto puesto, d.localidad localidad
FROM empleado e
JOIN departamento d
...
SELECT d.nombre_depto nombre_depto, d.localidad localidad, COUNT(e.cod_emp) cantidad_empleados
FROM departamento d
JOIN empleado e
...
-- ‘Mito Barchuk’.
SELECT e.nombre nombre, e.salario salario, d.nombre_depto nombre_depto
FROM empleado e
JOIN departamento d
    ON e.depto_nro = d.depto_nro
    WHERE e.puesto = (
