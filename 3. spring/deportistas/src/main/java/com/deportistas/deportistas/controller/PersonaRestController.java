...
import com.deportistas.deportistas.classes.PersonaDTO;
...
public class PersonaRestController {
    ...
    private List<Deporte> obtenerDeportes(@PathVariable("name") String nombre) {
        if(deportes.stream().findAny().isPresent()) {
            var deporteEncontrado = deportes.stream().filter(deporte -> deporte.getNombre().equals(nombre));
            return deporteEncontrado.toList();
        }
        return null;
    }*/
    //Hacer el dto a través del constructor
