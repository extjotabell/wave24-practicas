...
public class PostService implements IPostService {
    ...
    @Override
    public LastestPostDTO getLatestPost(Integer userId, String orderRequest) {
        ...
        usersFollowed.forEach((uf) -> postRepository.getAllPostsById(uf.getId()).forEach(post -> {
        ...
        if (lastestPostDTO.posts().isEmpty()) {
            throw new EmptyListException("No posts found from the last two weeks");
        }
        switch (order) {
            case "date_asc" -> lastestPostDTO.posts().sort(Comparator.comparing(PostWithIdDTO::date));
            case "date_desc" ->
