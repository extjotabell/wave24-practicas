...
public class UserServiceTest {
    ...
    @Test
    @DisplayName("Test that validates the same user can't be unfollowed")
    public void unfollowBadRequest() {
        // Arrange
        int userId = 101; // Existing user ID
        int sameUserId = 101;
        ...
        Assertions.assertThrows(
                BadRequestException.class,
                () -> userService.unfollow(userId, sameUserId)
        );
    }

    @Test
    @DisplayName("Test that validates if a user is not followed")
    public void unfollowUserNotFollowedTest() {
        // Arrange
        int userId = 101; // Existing user ID
        int idNotFollowed = 104;

        Mockito.when(userRepository.findById(userId)).thenReturn(Optional.of(UserConstants.USER1));
        Mockito.when(userRepository.findById(idNotFollowed)).thenReturn(Optional.of(UserConstants.USER4));

        // Act & Assert
        Assertions.assertThrows(
                UserNotFollowedException.class,
                () -> userService.unfollow(userId, idNotFollowed)
        );
    }
    ...
    @Test
    @DisplayName("Test that validates the correct functionality of alphabetical order descending")
    public void getFollowedBadRequestTest() {
        // Arrange
        int userId = 101;
        String order = "other";
