...
public class UserService implements IUserService {
    ...
    @Override
    public UserFollowersDto searchUserFollowers(Integer userId, String order) {
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        ...
        return modelMapper.convertUserFollowersToDto(user, users.stream()
                .map(modelMapper::convertUserToDto)
                .collect(Collectors.toList()));

    }

    public UserFollowedByDto getSellerFollowedByUser(Integer userId, String order) {
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        ...
        return modelMapper.convertUserFollowedToDto(user, followedList.stream()
                .map(modelMapper::convertUserToDto)
                .collect(Collectors.toList()));
